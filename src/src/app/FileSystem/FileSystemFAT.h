...
// The partition label matters: FFat.begin() defaults to a partition named
// "ffat", while partitions_16mb.csv names it "storage". Without the label
// the mount silently fails and every write is refused.
...
public:
    ...
    bool mkdir(const char *path)
    {
        ...
        return FFat.exists(path) || FFat.mkdir(path);
    }
